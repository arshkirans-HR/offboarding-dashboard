...
export async function POST(request: NextRequest) {
  try {
    ...
    // Close the modal immediately (Slack requires response within 3s)
    // We'll process the rest and send a follow-up message
    const responsePromise = processOffboarding({
      fullName,
      employeeEmail,
      department,
      jobTitle,
    ...
    responsePromise.catch((err) => {
      console.error('Background offboarding processing error:', err);
    });
